...
- 2026-07-13 · fable · Blender MCP addon: enable auto-start in the addon prefs (Edit > Preferences > Add-ons > MCP) so interactive MCP tools connect whenever the GUI is open; agents currently only get CLI mode
...
- 2026-07-06 · fable · the root Cargo.toml had a toys/* member glob that broke the first real cargo build (cargo requires every glob match to be a crate; toys/ is polyglot). Fixed to an explicit member list — add your rust toy to members. AGENTS.md updated to match.

- 2026-06-17 · agent · uv workspace: root project has package=false, so 'uv sync' alone does NOT install workspace member deps into .venv. Two working approaches: (1) add shared deps to root [project].dependencies (what primordia does with numpy), or (2) run 'uv sync --all-packages'. Plain 'uv sync' only resolves the root project's own deps. A toy with non-empty dependencies but no root-level dep entry will import-fail under 'uv run'.

- 2026-06-15 · claude · opentui (@opentui/core) works great for animated TUI toys with real graphics: FrameBufferRenderable + fb.setCell(x,y,'▀',fgRGBA,bgRGBA) gives two color pixels per char row (half-block trick) = crisp raster maps in the terminal. renderer.setFrameCallback + renderer.start() drives a continuous sim loop; renderer.keyInput.on('keypress', k=>k.name) for live input. Run TS toys with deps via bun, but install through pnpm at root so the dep lands in the shared pnpm-lock.yaml (the toys/* glob picks it up). RGBA.fromInts(r,g,b,a) takes 0-255. See toys/mythwright.

- 2026-06-15 · claude · tools/spotify.sh rewritten to use the Spotify Web API token (~/.cache/spotify-player/user_client_token.json) directly for ALL ops — reads, search, queue, transport. Previously it shelled out to the spotify_player daemon for now/queue/search, which fails with 'try to connect to a client / No cached credentials' whenever the daemon has no connected device (the common case — tetraslam's real playback runs on the desktop/phone, not the daemon). The Web API token has every needed scope and works as long as ANY device is active. playerctl/MPRIS is now just a fallback for 'now'. Added 'token-check'. If reads ever break: run 'tools/spotify.sh token-check'; if the token cache is missing, launch spotify_player once to re-auth.
